...
class AnovaWidget(QWidget, MessengerListener):
    ...
    def _update(self):
        selected_dependent_variable_names = self.ui.tableWidgetDependentVariable.get_selected_variable_names()
        if len(selected_dependent_variable_names) == 0:
            Notification(text="Please select dependent variable.", parent=self, duration=2000).show_notification()
            return

        dependent_variable = selected_dependent_variable_names[0]
    ...
    def _analyze_ancova(self, dependent_variable: str):
        ...
        factor_name = selected_factor_names[0]
        selected_covariates = self.ui.tableWidgetCovariates.get_selected_variable_names()
