...
def _extract_meal_events(
    df: pd.DataFrame,
    drinkfeed_data_settings: DrinkFeedDataSettings,
    drink1_present: bool,
    feed1_present: bool,
    drink2_present: bool,
    feed2_present: bool,
    ...
    timedelta = pd.Timedelta(
        hours=drinkfeed_data_settings.intermeal_interval.hour,
        minutes=drinkfeed_data_settings.intermeal_interval.minute,
        seconds=drinkfeed_data_settings.intermeal_interval.second,
    )
    ...
    if drink1_present:
        meal_events_df = _find_invalid_episodes(
            meal_events_df, "Drink1", drinkfeed_data_settings.drinking_minimum_amount
        )
    if feed1_present:
        meal_events_df = _find_invalid_episodes(meal_events_df, "Feed1", drinkfeed_data_settings.feeding_minimum_amount)

    if drink2_present:
        meal_events_df = _find_invalid_episodes(
            meal_events_df, "Drink2", drinkfeed_data_settings.drinking_minimum_amount
        )
    if feed2_present:
        meal_events_df = _find_invalid_episodes(meal_events_df, "Feed2", drinkfeed_data_settings.feeding_minimum_amount)

    if drink_present:
        meal_events_df = _find_invalid_episodes(
            meal_events_df, "Drink", drinkfeed_data_settings.drinking_minimum_amount
        )
    if feed_present:
        meal_events_df = _find_invalid_episodes(meal_events_df, "Feed", drinkfeed_data_settings.feeding_minimum_amount)
...
def _process_box(
    ...
    meal_details_settings: DrinkFeedDataSettings,
    start_timestamp: pd.Timestamp,
):
