...
class NWayAnovaWidget(QWidget):
    ...
    def _update(self):
        ...
        columns = self.datatable.get_default_columns() + list(self.datatable.dataset.factors) + [dependent_variable]
        ...
        criteria = "*".join([f"C({name})" for name in factor_names])
        formula = f"{dependent_variable} ~ {criteria}"
        model = ols(formula, data=df).fit()
        anova = sm.stats.anova_lm(model, typ=2).round(5)
