...
**`.claude/CLAUDE.md`**, the 5 auto-loaded **`.claude/rules/*.md`**, and the 14-file developer
reference **`docs/dev/*.md`**. These are prose *mirrors* of the source — when code changes and the
...
> **IMPORTANT:** the **code is the source of truth**; `CLAUDE.md`, `rules/*`, and `docs/dev/*` are
...
- **In scope:** the hand-maintained prose — `.claude/CLAUDE.md`, the 5 `.claude/rules/*.md`
  (`code-style.md`, `commands.md`, `project-structure.md`, `testing.md`, `toolbox-widget-pattern.md`),
  and the 14 `docs/dev/*.md` (`01-architecture` … `13-packaging-deployment`, `README.md`). Realign
...
2. **Apply the doc edits** for each changed item per the **code→doc map**:
   ...
   - `toolbox/toolbox_registry.py`, `toolbox/toolbox_widget_base.py`, `toolbox/__init__.py`, any widget →
     `docs/dev/08-toolbox.md` + `.claude/rules/toolbox-widget-pattern.md` + CLAUDE.md "Add a toolbox analysis widget".
   ...
   - conventions (Python version, deps, lint) → `docs/dev/11-conventions.md` + `.claude/rules/code-style.md`.
   - extending recipes → `docs/dev/12-extending.md`.
   - `Taskfile.yml`, packaging/PyInstaller/Flatpak → `docs/dev/13-packaging-deployment.md` + `.claude/rules/commands.md`.
...
4. **CLAUDE.md + rules.** Realign the terse agent-facing files only where the change touches a fact
   they state — keep edits surgical; CLAUDE.md and the rules are companions, not full copies of
   `docs/dev`.
...
- **Always re-read the code at run time.** `docs/`/`rules`/CLAUDE.md are a guide to verify, not ground
